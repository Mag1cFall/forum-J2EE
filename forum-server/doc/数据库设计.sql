...
CREATE TABLE `users` (
  ...
  `role` ENUM('MEMBER', 'MODERATOR', 'ADMIN') NOT NULL DEFAULT 'MEMBER' COMMENT '角色',
...
CREATE TABLE `user_votes` (
  ...
  CONSTRAINT `chk_vote_target` CHECK (`post_id` IS NOT NULL OR `comment_id` IS NOT NULL)
) COMMENT='用户投票记录表';

-- 3. 插入初始数据
INSERT INTO `users` (`username`, `password`, `role`, `status`) VALUES
('admin', 'admin123', 'ADMIN', 'ACTIVE'),
('moderator', 'mod123', 'MODERATOR', 'ACTIVE'),
('member1', 'mem123', 'MEMBER', 'ACTIVE'),
('member2', 'mem123', 'MEMBER', 'ACTIVE'),
('blockeduser', 'blocked123', 'MEMBER', 'BLOCKED');

INSERT INTO `boards` (`name`, `description`, `moderator_id`) VALUES
('技术交流', '讨论Java, Python, C++等编程技术', (SELECT id FROM users WHERE username = 'moderator')),
('生活分享', '分享日常生活的点点滴滴', NULL),
('站务公告', '发布论坛的重要通知', (SELECT id FROM users WHERE username = 'admin'));

INSERT INTO `posts` (`title`, `content`, `author_id`, `board_id`, `is_pinned`) VALUES
('【置顶】论坛使用规范', '请大家文明交流，遵守社区规则。', (SELECT id FROM users WHERE username = 'admin'), (SELECT id FROM boards WHERE name = '站务公告'), TRUE),
('Java是最好的语言吗？', '如题，大家来辩论一下，理性讨论，禁止人身攻击。', (SELECT id FROM users WHERE username = 'member1'), (SELECT id FROM boards WHERE name = '技术交流'), FALSE),
('今天天气真好', '阳光明媚，适合出去走走。', (SELECT id FROM users WHERE username = 'member2'), (SELECT id FROM boards WHERE name = '生活分享'), FALSE),
('Python学习求助', '请问Python的异步编程应该怎么入门？', (SELECT id FROM users WHERE username = 'member1'), (SELECT id FROM boards WHERE name = '技术交流'), FALSE);

INSERT INTO `comments` (`content`, `author_id`, `post_id`) VALUES
('同意，Java天下第一！', (SELECT id FROM users WHERE username = 'member2'), (SELECT id FROM posts WHERE title = 'Java是最好的语言吗？')),
('我还是觉得Python更简洁。', (SELECT id FROM users WHERE username = 'moderator'), (SELECT id FROM posts WHERE title = 'Java是最好的语言吗？')),
('沙发！', (SELECT id FROM users WHERE username = 'admin'), (SELECT id FROM posts WHERE title = '今天天气真好'));
