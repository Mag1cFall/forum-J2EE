...
public class ConsoleView {
	...
	private void showMainMenu() {
		...
		switch (choice) { // 根据选择执行不同操作
			...
			case 3:
				...
				break;
			default:
				System.out.println(RED + "无效的输入，请重试。" + RESET); // 处理无效输入
		}
	}

	// 显示用户菜单（已登录状态）
	private void showUserMenu() {
		String userInfo = "你好, " + CYAN + BOLD + currentUser.getUsername() + RESET + " (" + MAGENTA
				+ currentUser.getRole() + RESET + ")!"; // 格式化用户信息
		...
		switch (choice) { // 根据选择执行操作
			...
			case 4:
				if (currentUser.getRole() == User.UserRole.ADMIN)
					handleAdminMenu(); // 管理员的第4项是管理菜单
				else
					handleLogout(); // 普通用户的第4项是登出
				break;
			case 5:
				if (currentUser.getRole() == User.UserRole.ADMIN)
					handleLogout(); // 管理员的第5项是登出
				else
					System.out.println(RED + "无效的输入。" + RESET);
				break;
			default:
				System.out.println(RED + "无效的输入。" + RESET);
		}
		...
		while (true) {
			...
			switch (choice) {
				...
				case 3:
					return; // 返回
				default:
					System.out.println(RED + "无效的输入，请重试。" + RESET);
			}
		}
	}
	...
	private void handleViewPost(int postId) {
		...
		while (true) { // 进入帖子操作循环
			...
			if (currentUser.getRole() == User.UserRole.ADMIN || (postOpt.get().getBoard().getModerator() != null
					&& postOpt.get().getBoard().getModerator().getId() == currentUser.getId())) { // 检查置顶权限
			...
			switch (choice) {
				...
				case 4: // 置顶/取消置顶
					if (currentUser.getRole() == User.UserRole.ADMIN || (postOpt.get().getBoard().getModerator() != null
							&& postOpt.get().getBoard().getModerator().getId() == currentUser.getId())) {
		...
		try {
			if ("post".equals(type)) { // 如果是给帖子投票
				if (voteType == 0)
					...
					postService.dislikePost(id, currentUser.getId());
			} else { // 如果是给评论投票
				if (voteType == 0)
					postService.likeComment(id, currentUser.getId());
				else
					postService.dislikeComment(id, currentUser.getId());
			}
	...
	private void handleListBoardsInternal() {
		...
		for (Board board : boards) {
			String moderatorName = (board.getModerator() != null) ? CYAN + board.getModerator().getUsername() + RESET
					: "无";
	...
	private void printPostDetailsBox(Post post) {
		...
		System.out
				.println(GREEN + "  赞: " + post.getLikes() + RESET + " / " + RED + "踩: " + post.getDislikes() + RESET); // 打印赞踩数
		System.out.println();
